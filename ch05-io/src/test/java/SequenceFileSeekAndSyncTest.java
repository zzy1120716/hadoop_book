...
public class SequenceFileSeekAndSyncTest {
    ...
    public void seekToRecordBoundary() throws IOException {
        ...
        assertThat(((IntWritable) key).get(), is(97));
    }
    ...
    public void syncFromNonRecordBoundary() throws IOException {
        reader.sync(345);
        assertThat(reader.getPosition(), is(2000L));
        assertThat(reader.next(key, value), is(true));
        assertThat(((IntWritable) key).get(), is(74));
    }

    @Test
    public void syncAfterLastSyncPoint() throws IOException {
        reader.sync(7316);
        assertThat(reader.getPosition(), is(7388L));
        assertThat(reader.next(key, value), is(false));  //对书中结果有疑问
    }
}
