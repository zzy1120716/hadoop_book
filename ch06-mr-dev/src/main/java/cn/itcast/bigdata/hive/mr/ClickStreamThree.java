...
public class ClickStreamThree {
	...
	static class ClickStreamMapper extends Mapper<LongWritable, Text, Text, WebLogBean> {
		...
		protected void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			if (fields.length < 9) return; //因为字符中可能含有预处理后留下的""字符
			...
			//只有有效记录才进入后续处理
			if (v.isValid()) {
				k.set(v.getRemote_addr());   //ip，bean
				context.write(k, v);
			}
		}
	}
	...
	static class ClickStreamReducer extends Reducer<Text, WebLogBean, NullWritable, Text> {
		...
		@Override
		protected void reduce(Text key, Iterable<WebLogBean> logbeans, Context context) throws IOException, InterruptedException {
			ArrayList<WebLogBean> beans = new ArrayList<WebLogBean>();

			// 先将一个用户的所有访问记录中的时间拿出来排序
			try {
				for (WebLogBean bean : logbeans) {
					WebLogBean webLogBean = new WebLogBean();
					try {
						BeanUtils.copyProperties(webLogBean, bean);  //将bean直接复制给webLogBean。
					} catch(Exception e) {
						e.printStackTrace();
					}
					beans.add(webLogBean);
				}
				//将bean按时间先后顺序排序  ---- 按照key进行排序，并且通过groupingcomparator进行获取最大值。
