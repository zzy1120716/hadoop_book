...
import com.zzy.hadoopbook.crunch.fn.CountValuesFn;
import com.zzy.hadoopbook.crunch.fn.InversePairFn;
import com.zzy.hadoopbook.crunch.fn.ToLowerFn;
...
public class PipelineExecutionTest implements Serializable {
    ...
    @Test
    public void testInspectSynchronous() throws Exception {
        String inputPath = tmpDir.copyResourceFileName("set1.txt");
        String outputPath = tmpDir.getFileName("out");
        Pipeline pipeline = new MRPipeline(getClass());
        PCollection<String> lines = pipeline.readTextFile(inputPath);
    ...
    public void testComplexPipeline() throws Exception {
        ...
        PCollection<String> iso2 = table2.groupByKey().parallelDo("iso", new DoFn<Pair<String, Iterable<String>>, String>() {
            @Override
            public void process(Pair<String, Iterable<String>> input, Emitter<String> emitter) {
                for (String s : input.second()) {
                    emitter.emit(s);
        ...
        PipelineExecution execution = pipeline.runAsync();
        execution.waitUntilDone();
        FileUtils.write(dotFile(), execution.getPlanDotFile(), "UTF-8");
    ...
    public void testParallelDoFusion() throws Exception {
        ...
        Pipeline pipeline = new MRPipeline(getClass());
        PCollection<String> lines = pipeline.readTextFile(inputPath);
        PCollection<String> t1 = lines.parallelDo("t1", IdentityFn.<String>getInstance(), strings());
        PCollection<String> t2 = t1.parallelDo("t2", IdentityFn.<String>getInstance(), strings());
    ...
    public void testSiblingFusion() throws Exception {
        ...
        PCollection<String> lines = pipeline.readTextFile(inputPath);
        PCollection<String> t1 = lines.parallelDo("t1", IdentityFn.<String>getInstance(), strings());
        PCollection<String> t2 = lines.parallelDo("t2", IdentityFn.<String>getInstance(), strings());
