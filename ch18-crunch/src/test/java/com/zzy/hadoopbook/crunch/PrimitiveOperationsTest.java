...
public class PrimitiveOperationsTest implements Serializable {
    ...
    public void testGrouping() throws Exception {
        ...
        c = b.groupByKey(2);
        String dumpc = dump(c);
        assertTrue(dumpc, "{(5,[apple]),(6,[banana,cherry])}".equals(dumpc) || "{(6,[banana,cherry]),(5,[apple])}".equals(dumpc));

        c = b.groupByKey(); // 因为值迭代器是单次使用的
    ...
    public void testPCollectionStats() throws Exception {
        PCollection<String> a = MemPipeline.typedCollectionOf(strings(), "cherry", "apple", "banana", "banana");
