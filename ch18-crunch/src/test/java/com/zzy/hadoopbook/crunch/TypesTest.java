...
import com.zzy.hadoopbook.crunch.records.WeatherRecord;
...
public class TypesTest implements Serializable {
    ...
    public void testAvroReflect() throws Exception {
        ...
        PTable<Pair<Integer, Integer>, WeatherRecord> table = records.by(new MapFn<WeatherRecord, Pair<Integer, Integer>>() {
            @Override
    ...
    public void testAvroReflectSecondarySort() throws Exception {
        ...
        PCollection<Tuple3<Integer, Integer, WeatherRecord>> triples = records.parallelDo(new DoFn<WeatherRecord, Tuple3<Integer, Integer, WeatherRecord>>() {
            @Override
            public void process(WeatherRecord input, Emitter<Tuple3<Integer, Integer, WeatherRecord>> emitter) {
        ...
        PCollection<Tuple3<Integer, Integer, WeatherRecord>> sorted = Sort.sortTriples(triples, Sort.ColumnOrder.by(1, Sort.Order.ASCENDING), Sort.ColumnOrder.by(2, Sort.Order.DESCENDING));
        PCollection<WeatherRecord> sortedRecords = sorted.parallelDo(new DoFn<Tuple3<Integer, Integer, WeatherRecord>, WeatherRecord>() {
            @Override
            public void process(Tuple3<Integer, Integer, WeatherRecord> input, Emitter<WeatherRecord> emitter) {
    ...
    @Test
    public void testWriteWritableToSequenceFileFails() throws Exception {
        String inputPath = tmpDir.copyResourceFileName("ints.txt");
        String outputPath = tmpDir.getFileName("out");
        Pipeline pipeline = new MRPipeline(getClass());
    ...
    public void testWriteTextToSequenceFile() throws Exception {
        ...
        PTable<NullWritable, Text> d = pipeline2.read(From.sequenceFile(outputPath, NullWritable.class, Text.class));
